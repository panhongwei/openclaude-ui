...
const binaryName = process.platform === 'win32' ? 'openclaude.exe' : 'openclaude'

const result = await Bun.build({
  entrypoints: ['./src/entrypoints/cli.tsx'],
  outdir: './dist',
  target: isBinary ? 'bun' : 'node',
  format: isBinary ? undefined : 'esm',
  compile: isBinary ? true : undefined,
  splitting: false,
  sourcemap: isBinary ? undefined : 'external',
  minify: false,
  naming: isBinary ? binaryName : 'cli.mjs',
  define: {
...
`,
        ...
        for (const mod of [
          ...
          'code-excerpt',
          'stack-utils',
          // In binary mode, stub packages that were previously external
          // but are NOT installed (dynamic imports in telemetry code).
          ...(isBinary ? [
            'sharp',
...
`,
        ...
        // In binary mode, stub optional packages that are not installed.
        // @aws-sdk/client-bedrock-runtime and credential-providers are installed
        // as transitive deps and must be bundled with real exports.
        if (isBinary) {
          for (const pkg of [
...
if (!result.success) {
  ...
  if (isBinary) {
    console.log(`✓ Built openclaude v${version} → dist/${binaryName} (standalone binary)`)
  } else {
