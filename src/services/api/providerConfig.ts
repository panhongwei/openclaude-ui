...
export type ProviderTransport = 'chat_completions' | 'codex_responses' | 'openai_responses'
...
function shouldUseOpenAIResponsesTransport(
  model: string,
  baseUrl: string | undefined,
): boolean {
  if (!baseUrl || !baseUrl.includes('api.openai.com')) return false
  if (isEnvTruthy(process.env.OPENCLAUDE_USE_RESPONSES_API)) return true
  const norm = (model.toLowerCase().split('?', 1)[0] ?? '').split('?', 1)[0] ?? ''
  return (
    norm.startsWith('o1') ||
    norm.startsWith('o3') ||
    norm.startsWith('o4') ||
    norm.startsWith('gpt-4.1') ||
    norm.startsWith('gpt-4.5')
  )
}
...
export function resolveProviderRequest(options?: {
  ...
  const transport: ProviderTransport =
    shouldUseCodexTransport(requestedModel, finalBaseUrl) ||
      ...
      ? 'codex_responses'
      : shouldUseOpenAIResponsesTransport(descriptor.baseModel, rawBaseUrl)
      ? 'openai_responses'
      : 'chat_completions'
